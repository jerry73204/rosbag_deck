...
void status_callback_adapter(const PlaybackStatus& status, RosbagDeckWrapper* wrapper) {
...
void message_callback_adapter(const BagMessage& message, RosbagDeckWrapper* wrapper) {
    if (wrapper->message_callback) {
        ...
        c_message.original_timestamp = cpp_to_c_timestamp(message.original_timestamp);
...
void rosbag_deck_free_message(rosbag_deck_message_t* message) {
    // Note: The actual strings and data are stored in the wrapper object and will be 
    // cleaned up when the wrapper is destroyed. This function is provided
    // for API completeness and future extensibility.
